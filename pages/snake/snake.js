  ...
  //操场
   ...
   move:function(){
    ...
    tapEnd: function(event){
        var horizon = (this.data.endx) ? (this.data.endx - this.data.startx) : 0;
        var vertical = (this.data.endy) ? (this.data.endy - this.data.starty) : 0;

        if(Math.abs(horizon) > 5 || Math.abs(vertical) > 5){
            var direction = (Math.abs(horizon) > Math.abs(vertical)) ? this.computeDir(1, horizon):this.computeDir(0, vertical);
            switch(direction){
            case 'left':
